...
export async function sendNotification({
  ...
  try {
    ...
    console.log(`Found ${subs.length} push subscriptions for user: ${recipientId}`);
    ...
    for (const s of subs) {
      try {
        ...
        console.error("Webpush error", err);
        // Remove invalid subscriptions (e.g., 410 Gone)
        if (err.statusCode === 410 || err.body?.includes("not a valid")) {
          try {
            await PushSubscription.deleteOne({ _id: s._id });
            console.log(`Removed invalid push subscription ${s._id}`);
          } catch (deleteErr) {
            console.error("Failed to delete invalid subscription", deleteErr);
          }
        }
