  ...
  return (
    <div className="max-w-5xl mx-auto p-6 rounded-xl bg-white dark:bg-slate-900 shadow-md sm:p-8 sm:mt-6">
      ...
      <section className="mb-8">
        <Checklist checklist={task?.checklist ?? []} onToggle={onToggleChecklist} />
      </section>
      ...
      <section className="mb-8 max-w-3xl">
        <h2 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">Description</h2>
        <p className="text-gray-900 dark:text-gray-200 whitespace-pre-wrap">{task.description || '-'}</p>
      </section>

      {/* Assignees */}
      <section className="mb-8 max-w-md">
        <h2 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">Assignees</h2>
      ...
      {userRole !== 'member' && (
        <div className="flex flex-wrap gap-4 justify-start">
          <button
            onClick={() =>
              router.push(
                `/dashboard/manage-tasks/${task._id}/update?projectName=${encodeURIComponent(
                  task.projectId
                )}`
              )
            }
